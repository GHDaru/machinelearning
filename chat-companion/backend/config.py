"""Configuração do backend, lida só de variáveis de ambiente.

Princípio V aplicado ao próprio projeto: nenhuma credencial no código. A chave
do modelo e a DATABASE_URL vivem só no ambiente. Defaults são seguros: sem
chave -> adapter echo; sem banco -> store em memória.
"""
...
RATE_LIMIT_TENTATIVAS = _int("RATE_LIMIT_TENTATIVAS", 60)     # tentativas de exercício por janela
RATE_LIMIT_WINDOW_S = _int("RATE_LIMIT_WINDOW_S", 300)        # janela em segundos
RATE_LIMIT_IP_FACTOR = _int("RATE_LIMIT_IP_FACTOR", 3)        # teto por IP = MSGS × fator
CONTEXT_WINDOW_TOKENS = _int("CONTEXT_WINDOW_TOKENS", 32000)  # janela exibida nos Bastidores
...
# contextos isolados (deploy onde só esta pasta é copiada): procura subindo até
# achar `livro/`; se não achar, o `corpus.json` empacotado cobre a busca.
def _find_repo_root() -> Path:
    ...
    for parent in aqui.parents:
        if (parent / "livro").is_dir():
            return parent
    return aqui.parent  # fallback seguro; corpus.json cobre o livro


REPO_ROOT = _find_repo_root()
# Corpus do livro empacotado (gerado por build_corpus.py) e banco de exercícios
# (gerado por publicar/exercicios.mjs) — o que o backend precisa do livro.
