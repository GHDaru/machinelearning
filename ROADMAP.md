...
| **D16** | **Dois objetivos declarados em capítulos que não os ensinam.** `v-2` O4 (decidir entre lote e tempo real pelo requisito) é ensinado pelo **`v-3`**, na seção "Servir" — inclusive com a frase "escolhidas pelo requisito e não pelo gosto", que é a redação do objetivo do outro capítulo. `v-3` O2 (implantar atrás de API com contrato e validação de entrada) não é coberto por nenhum | escrever exercício aqui seria cobrar o que o capítulo não ensina. A saída é editorial — **o conteúdo sobe ou o verbo desce** —, e passa por decidir a quem pertence a decisão de arquitetura de serving: ao capítulo de sistemas ou ao de operação |
