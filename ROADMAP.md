...
| **D27** | **A escada do II.2 vira piso depois do cartão 15** — a inclinação da dificuldade é **−0,0022 por cartão**, com média 3,11 na primeira metade e **3,11** na segunda. Não é ruído em torno de uma subida: é ausência de subida. O desvanecimento aparece nos cartões 5, 7, 12, 14 e 23 e **nunca mais**; dos 38 exercícios, 21 são "escolha uma", e a única resposta que o aluno produz está a 84,2% do caminho | achado por crítico cego em 2026-09-05, contra o notebook do capítulo 10 do Géron, cuja inclinação é **+0,0035 por célula**. O custo é concreto: a segunda metade carrega o conteúdo mais difícil do capítulo, confundimento e colinearidade e R² como armadilha, e o cobra no nível de reconhecimento. O aluno acerta entre quatro alternativas sem nunca **produzir** um diagnóstico, e sai achando que sabe. O conserto proposto pelo crítico é a espiral: transformar a limonada num laboratório que reabre a cada cartão, uma variável nova por vez, que é o mecanismo do Géron aplicado ao dado que o capítulo já tem |
| **D28** | **O que o navegador desenha num canvas é invisível a todos os portões que leem a fonte** — o painel do laboratório do cartão 4 mostra `R²` na tela, e o gate de pré-requisito declara R² apresentado no cartão 23. São **50 pontos percentuais** de inversão que o portão não vê, porque `laboratorios.js` desenha o rótulo em runtime | é a terceira vez que este mesmo ponto cego morde. A primeira foram as 14 siglas que continuam nuas na D24, desenhadas no canvas onde o passe de `<abbr>` não chega. A segunda é esta. O padrão vale para qualquer portão de fonte, presente e futuro, e o conserto não é remendar cada um: é o vocabulário do capítulo alcançar o que o tema desenha, ou os portões passarem a ler o DOM montado em vez do Markdown |
