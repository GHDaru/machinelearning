...
| **D24** | **Quase metade das siglas do livro nasce nua, e a interatividade é a causa** — o `build.mjs` embrulha sigla em `<abbr>` com a expansão no `title`, mas o `:::exercicio` e a `:::interacao` viram HTML **antes** desse passe, então nada dentro deles é tocado. Medido: **223 ocorrências dentro de bloco ficam nuas contra 237 fora que viram `<abbr>`**, e o `II.2` tem 15. O Princípio VIII diz que a primeira ocorrência de uma sigla vem por extenso, e o capítulo usa "AUC" quatro vezes sem expandir nenhuma | medido em 2026-09-01, ao investigar por que o gate do glossário não cobra sigla. Ele dispensa as 25 siglas da tabela **de propósito**, e a justificativa escrita é "o motor já a embrulha em `<abbr>`" — verdadeira em 35 páginas e **falsa exatamente onde o texto é interativo**. O `II.2` tem zero `<abbr>`. É isenção apoiada em mecanismo que não chega lá, e quanto mais cartão o livro ganha, maior a fatia que escapa |
| **D25** | **Nenhuma figura em 39 cartões, e a convexidade é entregue como palavra** — o cartão 9 apoia o capítulo inteiro em *"uma superfície convexa em (a,b), uma tigela; tigela tem um fundo, e só um"*, e é dessa afirmação que sai o direito de derivar, igualar a zero e chamar o resultado de mínimo. Os cartões 10 a 14 então pedem álgebra numa tela de 360px com um parabolóide de cabeça, a partir de um substantivo | achado por crítico cego em 2026-09-01, comparando com o módulo de regressão linear do Google, que mostra a tigela três vezes e pareia seis curvas de perda com a reta que cada uma produz. O conserto proposto não é ilustração: é um segundo painel no laboratório do cartão 8, com o plano (a,b) em curvas de nível e o ponto do leitor sobre ele, deixando rastro. O leitor arrasta a alça e vê o próprio ponto **descer a tigela** — ela deixa de ser palavra e vira coisa que ele moveu |
