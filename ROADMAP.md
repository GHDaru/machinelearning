...
| **D11** | **2 objetivos sem exercício próprio** — eram **20** em 2026-08-12, e não 2, como esta linha declarou até então | achado pela auditoria de Bloom. O gate do `exercicios.mjs` **cobra**: órfão novo quebra o build, e a exceção sai da lista no mesmo commit em que a dívida é paga. **18 pagos** pelo [ADR 0012](adr/0012-verificacao-como-superficie-corrigida.md). Os 2 que restam **não são falta de exercício, e sim de conteúdo** — ver D16 |
| **D16** | **Um objetivo declarado num capítulo que não o ensina** — eram dois. O `v-2` O4 (decidir entre lote e tempo real pelo requisito) foi **PAGO** em 2026-08-13 pelo [ADR 0016](adr/0016-a-quem-pertence-a-escolha-da-forma-de-servico.md): escolher a forma de serviço é desenho, o `v-2` ganhou a seção que faltava, e o `v-3` devolveu a tabela que lá era órfã de objetivo. Restou o `v-3` O2 (implantar atrás de API com contrato e validação de entrada), que nenhum capítulo cobre | escrever exercício aqui seria cobrar o que o capítulo não ensina. A saída é editorial: **o conteúdo sobe ou o verbo desce**. O caso do `v-2` ensinou por onde decidir — a pergunta não era de arrumação, e sim de evidência: a escolha não está em nenhuma das duas fontes-base, e a indústria a trata como assunto de projeto |
