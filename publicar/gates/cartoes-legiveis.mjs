...
//   ≥ 80% dos cartões com pelo menos um exercício ou laboratório
//
...
for (const nome of paginas) {
  ...
  if (foraAltura.length || foraPalavras.length || razao > LIMITES.razaoMax || comInter < LIMITES.cobertura) {
...
if (falhas.length) {
  ...
  for (const f of falhas) {
    ...
    console.error(`     com exercício ou laboratório: ${(100 * f.comInter).toFixed(0)}% (piso ${100 * LIMITES.cobertura}%)`);
...
console.log(`✓ ${comCartoes} página(s) com modo cartão: todo cartão fecha entre ${LIMITES.alturaMin} e ${LIMITES.alturaMax}px e entre ${LIMITES.palavrasMin} e ${LIMITES.palavrasMax} palavras.`);
